...
public class StageTown extends Stage implements Init {
	...
	public boolean check(int y, int x, String interaction) {
		...
		if (Map.map.get(y).get(x).equals(GUILD) && interaction.equals("q")) {
		...
		if (Map.map.get(y).get(x).equals(POTAL_CAVE) && GameManager.isOpenCave && interaction.equals("q")) {
			...
			GameManager.getInstace().showLoading();
		} else if (Map.map.get(y).get(x).equals(POTAL_CAVE) && !GameManager.isOpenCave) {
			String text = GameManager.red + "아직 해당 던전을 이용할 수 있는 자격이 없습니다." + GameManager.exit;
			GameManager.getInstace().showText(text, 1000);
			return false;
		}

		if (Map.map.get(y).get(x).equals(POTAL_CASTLE) && GameManager.isOpenCastle) {
			String text = "던전[성채]로 이동합니다.\n";
			GameManager.getInstace().showText(text, 1000);
			GameManager.nextStage = "CASTLE";
			GameManager.getInstace().showLoading();
		} else if (Map.map.get(y).get(x).equals(POTAL_CASTLE) && !GameManager.isOpenCastle) {
			String text = GameManager.red + "아직 해당 던전을 이용할 수 있는 자격이 없습니다." + GameManager.exit;
			GameManager.getInstace().showText(text, 1000);
			return false;
		}
